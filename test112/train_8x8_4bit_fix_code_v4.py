...
ncomp = 4
sc_int= 20
checkpoint_dir='./model_8x8_4bit_fix_code_v4/'
graph_dir='./graph_8x8_4bit_fix_code_v4/'

import h5py
f_train=h5py.File('../raw/demosaiced_reconstruct_fix_code_binary.h5','r')
x_train=f_train['data'][7000:12000]

f_test=h5py.File('../raw/demosaiced_reconstruct_fix_code_binary.h5','r')
x_test=f_test['data'][12000:14000]
...
with tf.name_scope('DATA'):
    Data_train = tf.placeholder(tf.float32,shape=[None,input_image_size,input_image_size,None])
    print(Data_train)
...
with tf.name_scope('Compress_Conv_f32'):
    compf32 = tf.cast(compint,tf.float32)/80.
    print(compf32)
...
with tf.Session() as sess:
    ...
    for i in range(global_step.eval(),num_of_iter+1):
        a = np.random.randint(0,4990,gradient_batch)
        ...
        if i%display_step == 0:
            ...
            print("4bit v4 80 Iteration %d, learning rate %g, weight rate %g"%(
