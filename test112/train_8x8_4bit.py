...
ncomp = 4
sc_int=50 
checkpoint_dir='./model_8x8_4bit/'
graph_dir='./graph_8x8_4bit/'
...
with tf.name_scope('comp_Wi_6bit'):
    compWi_6bit = compWi*16
    compWi_6bit = tf.cast(compWi_6bit,tf.int8)#2nd constraint;max is 31
    compWi_6bit = compWi_6bit/16
    print("\nlimit it an  6bit integer\n" + str(compWi_6bit))

with tf.name_scope('comp_Wf32'):
    compWf32 = tf.cast(compWi_6bit,tf.float32)
