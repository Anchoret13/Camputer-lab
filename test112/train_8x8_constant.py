...
ncomp = 4
sc_int=10000.
checkpoint_dir='./model_8x8_constant/'
graph_dir='./graph_8x8_constant/'
...
x_test=f_test['data'][23500:]

f_code=h5py.File('./code_begin.h5','r')
code=f_code['data'][:]
...
with tf.name_scope('comp_Wi'):
    compWi = compW*sc_int
    compWi = tf.cast(compWi,tf.int16)
    print("\nMake it an integer\n" + str(compWi))
...
with tf.Session() as sess:
    ...
    if ckpt and ckpt.model_checkpoint_path:
        ...
        print('Loading....................')
        test_loss = Loss.eval(feed_dict={Data_train:x_train[0:2,:,:,:]})
    ...
    for i in range(global_step.eval(),num_of_iter+1):
        ...
        if i%display_step == 0:
            ...
            test_loss = Loss.eval(feed_dict={Data_train:x_test[0:2,:,:,:]})
            train_psnr = Psnr.eval(feed_dict={Data_train:x_train[a,:,:,:]})
            test_psnr = Psnr.eval(feed_dict={Data_train:x_test[0:2,:,:,:]})
